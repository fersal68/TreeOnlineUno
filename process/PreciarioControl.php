...
$columns = ['ID_PRE', 'ID_ART', 'ITEM', 'DESCRIPCION', 'PRECIO','FECHA_VIG','FECHA_VTO','USUARIO','FECHA_ALTA'];
// Nombre de la tabla
$table = "prodes.preciario";
...
if ($num_rows > 0) {
    while ($row = $resultado->fetch(PDO::FETCH_ASSOC)) {
        ...
        $output['data'] .= '<td><a class="btn btn-warning btn-sm" href="?sec=ItemAddEdit&id=' . $row['ID_PRE'] . '">
            <i class="ri-edit-2-fill"></i> Editar</a></td>';
        $output['data'] .= '<td><a class="btn btn-danger btn-sm" onclick="confirmDeshabilitar(' . $row['ID_PRE'] . ')">
